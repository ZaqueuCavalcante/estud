    ...
    <template #body>
      <div class="space-y-6">
        <div class="flex gap-2">
          ...
          >
            <template #default>
              <span v-if="selectedCampusIds.length" class="truncate">
                {{ selectedCampusIds.length }}
                {{ selectedCampusIds.length === 1 ? 'campus selecionado' : 'campi selecionados' }}
              </span>
              <span v-else class="truncate text-dimmed">Pesquisar campus para vincular...</span>
            </template>
          </USelectMenu>
          <UButton
            label="Vincular"
