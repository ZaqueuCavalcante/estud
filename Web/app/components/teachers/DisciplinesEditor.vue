...
import type { TeacherDisciplineItem } from '~/types/teachers'

const props = defineProps<{ teacherId: number, disciplines: TeacherDisciplineItem[] }>()
...
const search = ref('')
const potential = ref<TeacherDisciplineItem[]>([])
...
const allDisciplines = computed(() => {
  const byId = new Map<number, TeacherDisciplineItem>()
...
async function fetchPotential() {
  loading.value = true
  try {
    const result = await $fetch<{ items: TeacherDisciplineItem[] }>(
      `${config.public.backendUrl}/teachers/${props.teacherId}/potential-disciplines`,
      { credentials: 'include' },
    )
...
async function save() {
  saving.value = true
  try {
    await $fetch(`${config.public.backendUrl}/teachers/${props.teacherId}/assign-disciplines`, {
      method: 'PUT',
      body: { disciplines: selectedIds.value },
      credentials: 'include',
    })

    toast.add({ title: 'Disciplinas do professor atualizadas', color: 'success' })
    cancelEditing()
  } catch (err: unknown) {
    toast.add({
  ...
  <section class="flex flex-col gap-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <p class="text-sm text-muted">
        As disciplinas que este professor está apto a lecionar.
      </p>
    ...
    <template v-if="!editing">
      ...
      <TableEmptyState
        ...
        message="Nenhuma disciplina vinculada a este professor"
