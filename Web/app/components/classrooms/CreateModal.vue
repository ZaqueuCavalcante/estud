...
// Uma sala sempre pertence a um campus: quando o modal é aberto de dentro de um
// campus o vínculo já está definido, e o select de campus some do formulário.
const props = defineProps<{ campusId?: number }>()
...
const { data: campiData } = await useFetch<GetCampiOut>(`${config.public.backendUrl}/campi`, {
  credentials: 'include',
  server: false,
  // Aberto de dentro de um campus, o select não aparece — nem precisa da lista.
  immediate: !props.campusId,
})
...
function resetForm() {
  formState.name = ''
  formState.campusId = props.campusId
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    toast.add({ title: 'Sala criada com sucesso', color: 'success' })
    open.value = false
    emit('created')
  } catch (err: unknown) {
    const msg = (err as { data?: { message?: string } })?.data?.message ?? 'Erro ao criar sala.'
  ...
  >
    <template #body>
      ...
      >
        ...
        <UFormField v-if="!props.campusId" label="Campus" name="campusId">
          <USelectMenu
