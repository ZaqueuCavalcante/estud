    ...
    <template #body>
      <div class="space-y-6">
        <div class="flex gap-2">
          ...
          >
            <template #default>
              <span v-if="selectedCourseIds.length" class="truncate">
                {{ selectedCourseIds.length }}
                {{ selectedCourseIds.length === 1 ? 'curso selecionado' : 'cursos selecionados' }}
              </span>
              <span v-else class="truncate text-dimmed">Pesquisar cursos para vincular...</span>
            </template>
          </USelectMenu>
          <UButton
            label="Vincular"
