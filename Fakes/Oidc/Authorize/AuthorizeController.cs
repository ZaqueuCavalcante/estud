...
namespace Estud.Fakes.Oidc.Authorize;
...
public class AuthorizeController : ControllerBase
{
    ...
    public IActionResult Authorize(
        ...
        [FromQuery(Name = "fake_nonce")] string? fakeNonce = null,
        [FromQuery(Name = "fake_code_challenge")] string? fakeCodeChallenge = null,
        ...
        [FromQuery(Name = "fake_email")] string? email = null,
        [FromQuery(Name = "fake_no_email")] bool noEmail = false,
        [FromQuery(Name = "fake_email_verified")] bool emailVerified = true,
        [FromQuery(Name = "fake_name")] string? name = null,
        [FromQuery(Name = "fake_userinfo_subject")] string? userInfoSubject = null)
    {
        if (error.HasValue())
        {
        ...
        if (!invalidCode)
        {
            OidcFakeUsers.ByCode[code] = new OidcFakeUser(
                noEmail ? null : email ?? loginHint,
                subject ?? Guid.NewGuid().ToString(),
                clientId,
                fakeNonce ?? nonce,
                fakeCodeChallenge ?? codeChallenge,
                emailVerified,
                name,
                userInfoSubject
            );
