...
{
    public async Task<OneOf<GetCoursePotentialDisciplinesOut, EstudError>> Get(int courseId, string? name)
    {
        var institutionId = ctx.RequestUser.InstitutionId;

        var course = await ctx.Courses.AsNoTracking().Include(c => c.Links)
            .FirstOrDefaultAsync(c => c.InstitutionId == institutionId && c.Id == courseId);
