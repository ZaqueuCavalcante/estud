...
public class UpdateCourseService(EstudDbContext ctx) : IEstudService
{
    ...
    public async Task<OneOf<UpdateCourseOut, EstudError>> Update(int courseId, UpdateCourseIn data)
    {
        if (V.Run(data, out var error)) return error;
        var institutionId = ctx.RequestUser.InstitutionId;

        var course = await ctx.Courses.FirstOrDefaultAsync(x => x.InstitutionId == institutionId && x.Id == courseId);
