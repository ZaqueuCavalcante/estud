...
public class UpdateRoleService(EstudDbContext ctx) : IEstudService
{
    ...
    public async Task<OneOf<UpdateRoleOut, EstudError>> Update(int roleId, UpdateRoleIn data)
    {
        if (V.Run(data, out var error)) return error;

        var institutionId = ctx.RequestUser.InstitutionId;

        var role = await ctx.Roles.FirstOrDefaultAsync(r => r.InstitutionId == institutionId && r.Id == roleId);
