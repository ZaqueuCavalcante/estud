...
public class UpdateClassroomService(EstudDbContext ctx) : IEstudService
{
    ...
    public async Task<OneOf<UpdateClassroomOut, EstudError>> Update(int classroomId, UpdateClassroomIn data)
    {
        if (V.Run(data, out var error)) return error;

        var institutionId = ctx.RequestUser.InstitutionId;

        var classroom = await ctx.Classrooms.FirstOrDefaultAsync(x => x.InstitutionId == institutionId && x.Id == classroomId);
