...
public class UpdateCampusService(EstudDbContext ctx) : IEstudService
{
    ...
    public async Task<OneOf<UpdateCampusOut, EstudError>> Update(int campusId, UpdateCampusIn data)
    {
        if (V.Run(data, out var error)) return error;

        var campus = await ctx.Campi.FirstOrDefaultAsync(x => x.InstitutionId == ctx.RequestUser.InstitutionId && x.Id == campusId);
