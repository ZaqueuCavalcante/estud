...
{
    public async Task<OneOf<GetTeacherDetailsOut, EstudError>> Get(int teacherId)
    {
        ...
        var teacher = await ctx.Teachers.AsNoTracking()
            ...
            .FirstOrDefaultAsync(t => t.InstitutionId == institutionId && t.Id == teacherId);
        if (teacher == null) return TeacherNotFound.I;

        var teacherName = teacher.Name;

        var classes = await ctx.Classes.AsNoTracking()
            .Where(c => c.InstitutionId == institutionId && c.Teachers.Any(t => t.Id == teacherId))
