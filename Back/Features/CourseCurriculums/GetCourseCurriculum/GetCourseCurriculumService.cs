...
{
    public async Task<OneOf<GetCourseCurriculumOut, EstudError>> Get(int courseCurriculumId)
    {
        var curriculum = await ctx.CourseCurriculums.AsNoTracking()
            .Include(c => c.Course)
            .Include(c => c.Links)
            .FirstOrDefaultAsync(c => c.InstitutionId == ctx.RequestUser.InstitutionId && c.Id == courseCurriculumId);
